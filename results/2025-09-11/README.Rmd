...
```{r diff-sites, message=FALSE, warning=FALSE}
    ...
    input_sig="sig.csv";
    em=fread(input)
#    for(j in setdiff(names(em), c("chrom", "start"))){
#        tmp=em[,.(chrom,start,get(j))]
#        tmp[,end:=as.integer(start+1)]
#        tmp[is.na(tmp)]=0;
#        fwrite(tmp[,.(chrom,start,end,V3)],file=paste0("bigdata/",j,".bedGraph.gz"),sep="\t",col.names=F);
#    }
...
knitr::kable( res[ order(-l2EW)][pvEW < 0.05][1:20], , caption = "Sig + EtoW Sites")
knitr::kable( res[ order(l2EW)][pvEW < 0.05][1:20], , caption = "Sig - EtoW Sites")
```

IGV SnapShots:
![pcmtd1](igv_snapshot_pcmtd1.png)
![st18](igv_snapshot_st18.png)


```{r diff-prom, message=FALSE, warning=FALSE}
...
fwrite(tt,"rna_promoter_emseq.csv")

```
[RNA_Promoter_EMseq](rna_promoter_emseq.csv)

### IGV snapshots
![Fos](igv_snapshot_fos.png) 
![Igf2bp1](igv_snapshot_igf2bp1.png)
![lin28b](igv_snapshot_lin28b.png)
![hmga2](igv_snapshot_hmga2.png )
