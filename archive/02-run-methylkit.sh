...
write.table(anno[anno$qvalue <= 0.05 & anno$meth.diff >= 25,],file=paste0(output,"_diff_25p_05q_hyper.tsv"),col.names=T,row.names=F,quote=F,sep="\t")
...
write.table(anno[anno$qvalue <= 0.05 & anno$meth.diff <= -25,],file=paste0(output,"_diff_25p_05q_hypo.tsv"),col.names=T,row.names=F,quote=F,sep="\t")
