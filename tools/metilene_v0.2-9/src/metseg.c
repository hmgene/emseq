...
void kstest(segment_t *seg , int a, int b, char mindiff, char mincpgs, char test,
 ...
 // fprintf(stdout,"pVALUE %f\n",p);
  ...
  //   || ks[2]
  //   ){
  //   fprintf(stderr, "fooooooks1 %f\n", ks[0]);
  // fprintf(stderr, "fooooooks2 %f\n", ks[1]);
  // fprintf(stderr, "fooooooks3 %f\n", ks[2]);
  // }
  ...
  while(!bl_segmentstackIsEmpty(&stack) || a != -1) {
    ...
    if(a != -1) {
      ...
      if(max->start > 0 && n <= m) {
        ...
        a = n;
        b = m;
        // illegal operation worked until gcc <= 11: KS = ks;
        memmove(KS, ks, sizeof(double)*3);
      } else {
        a = -1;
      }
